...
export const tasks = sqliteTable(
  ...
  (table) => [
    ...
    check(
      'tasks_target_check',
      sql`${table.defaultPomodoroTarget} BETWEEN 1 AND 99`,
    ),
    check('tasks_version_check', sql`${table.version} > 0`),
    check(
      'tasks_preset_check',
      sql`${table.defaultTimerPreset} IN ('25-5', '50-10', 'custom')`,
    ),
  ],
);
...
export const dailyTasks = sqliteTable(
  ...
  (table) => [
    index('daily_tasks_user_date_sort_idx').on(
      table.userId,
      table.date,
      table.sortOrder,
    ),
    ...
    check(
      'daily_tasks_target_check',
      sql`${table.pomodoroTarget} BETWEEN 1 AND 99`,
    ),
    check('daily_tasks_completed_check', sql`${table.pomodoroCompleted} >= 0`),
    check('daily_tasks_version_check', sql`${table.version} > 0`),
    check(
      'daily_tasks_preset_check',
      sql`${table.timerPreset} IN ('25-5', '50-10', 'custom')`,
    ),
    check(
      'daily_tasks_status_check',
      sql`${table.status} IN ('pending', 'active', 'awaiting_confirmation', 'completed')`,
    ),
  ],
);
...
export const focusSessions = sqliteTable(
  'focus_sessions',
  {
    ...
    userId: text('user_id')
      .notNull()
      .references(() => users.id, { onDelete: 'cascade' }),
    dailyTaskId: text('daily_task_id').references(() => dailyTasks.id, {
      onDelete: 'set null',
    }),
  ...
  (table) => [
    ...
    check(
      'focus_sessions_phase_check',
      sql`${table.phase} IN ('focus', 'short_break', 'long_break')`,
    ),
    check(
      'focus_sessions_result_check',
      sql`${table.result} IN ('completed', 'interrupted', 'abandoned')`,
    ),
    check(
      'focus_sessions_duration_check',
      sql`${table.plannedSeconds} > 0 AND ${table.effectiveSeconds} >= 0`,
    ),
    check(
      'focus_sessions_time_check',
...
export const activeTimer = sqliteTable(
  'active_timer',
  {
    id: text('id').primaryKey(),
    singletonKey: integer('singleton_key').notNull().default(1),
  ...
  (table) => [
    ...
    check(
      'active_timer_phase_check',
      sql`${table.phase} IN ('focus', 'short_break', 'long_break')`,
    ),
    check(
      'active_timer_status_check',
      sql`${table.status} IN ('running', 'paused')`,
    ),
    check(
      'active_timer_duration_check',
      sql`${table.plannedSeconds} > 0 AND ${table.accumulatedPausedSeconds} >= 0`,
    ),
...
export const conflicts = sqliteTable(
  'conflicts',
  {
    ...
    localPayload: text('local_payload', { mode: 'json' })
      .$type<JsonObject>()
      .notNull(),
    serverPayload: text('server_payload', { mode: 'json' })
      .$type<JsonObject>()
      .notNull(),
  ...
  (table) => [
    ...
    check(
      'conflicts_type_check',
      sql`${table.conflictType} IN ('delete_modify', 'complete_restore', 'archive_add_today', 'timer_divergence')`,
    ),
    check(
      'conflicts_entity_type_check',
      sql`${table.entityType} IN ('task', 'dailyTask', 'focusSession', 'activeTimer', 'settings')`,
    ),
    check(
...
export const syncOperations = sqliteTable(
  'sync_operations',
  {
    ...
    conflictId: text('conflict_id').references(() => conflicts.id, {
      onDelete: 'set null',
    }),
  ...
  (table) => [
    ...
    check(
      'sync_operations_result_check',
      sql`(
    (${table.status} = 'applied' AND ${table.entityVersion} IS NOT NULL AND ${table.conflictId} IS NULL AND ${table.errorCode} IS NULL AND ${table.errorMessage} IS NULL)
    OR (${table.status} = 'conflict' AND ${table.conflictId} IS NOT NULL AND ${table.errorCode} IS NULL AND ${table.errorMessage} IS NULL)
    OR (${table.status} = 'rejected' AND ${table.conflictId} IS NULL AND ${table.errorCode} IS NOT NULL AND ${table.errorMessage} IS NOT NULL)
  )`,
    ),
    check(
      'sync_operations_entity_type_check',
      sql`${table.entityType} IN ('task', 'dailyTask', 'focusSession', 'activeTimer', 'settings')`,
    ),
    check(
      'sync_operations_type_check',
      sql`(
...
export const syncChanges = sqliteTable(
  ...
  (table) => [
    ...
    check(
      'sync_changes_type_check',
      sql`${table.changeType} IN ('upsert', 'delete')`,
    ),
    check(
      'sync_changes_entity_type_check',
      sql`${table.entityType} IN ('task', 'dailyTask', 'focusSession', 'activeTimer', 'settings')`,
    ),
    check(
      'sync_changes_payload_check',
      sql`(
...
export const settings = sqliteTable(
  'settings',
  {
    ...
    customShortBreakMinutes: integer('custom_short_break_minutes')
      .notNull()
      .default(8),
    customLongBreakMinutes: integer('custom_long_break_minutes')
      .notNull()
      .default(20),
  ...
  (table) => [
    ...
    check(
      'settings_preset_check',
      sql`${table.defaultPreset} IN ('25-5', '50-10', 'custom')`,
    ),
    check(
      'settings_focus_check',
      sql`${table.customFocusMinutes} BETWEEN 1 AND 180`,
    ),
    check(
      'settings_short_break_check',
      sql`${table.customShortBreakMinutes} BETWEEN 1 AND 60`,
    ),
    check(
      'settings_long_break_check',
      sql`${table.customLongBreakMinutes} BETWEEN 1 AND 120`,
    ),
    check(
      'settings_interval_check',
      sql`${table.longBreakInterval} BETWEEN 1 AND 12`,
    ),
  ],
);
