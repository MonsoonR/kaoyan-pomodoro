CREATE TEMP TABLE `_migration_0009_guard` (`ok` integer NOT NULL CHECK (`ok` = 1));--> statement-breakpoint
INSERT INTO `_migration_0009_guard` (`ok`)
SELECT CASE WHEN NOT EXISTS (
  SELECT 1
  FROM `sync_operations` operation
  LEFT JOIN `devices` device ON device.`id` = operation.`device_id`
  LEFT JOIN `conflicts` conflict ON conflict.`id` = operation.`conflict_id`
  WHERE device.`id` IS NULL
     OR device.`user_id` <> operation.`user_id`
     OR (operation.`conflict_id` IS NOT NULL AND (
       conflict.`id` IS NULL OR conflict.`user_id` <> operation.`user_id`
     ))
) THEN 1 ELSE 0 END;--> statement-breakpoint
DROP TABLE `_migration_0009_guard`;--> statement-breakpoint
